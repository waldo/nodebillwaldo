(function() {
  ...
  exports.calculate = function(bills) {
    var amt, bill, compare, groupHack, groupedPayee, groupedPayer, len, name, neg, negatives, payee, pos, positives, res, summary, val, _i, _j, _k, _l, _len, _len2, _len3, _len4, _ref;
    res = {};
    groupHack = {
      chris: "chris+liz",
      craig: "jon+craig",
    ...
    for (_i = 0, _len = bills.length; _i < _len; _i++) {
      ...
      res[groupedPayer].paid += parseFloat(bill.amount);
      res[groupedPayer].owed = res[groupedPayer].paid - res[groupedPayer].share;
      _ref = bill.payees;
      for (_j = 0, _len2 = _ref.length; _j < _len2; _j++) {
        payee = _ref[_j];
        ...
        res[groupedPayee].share += parseFloat(bill.amount) / len;
        res[groupedPayee].owed = res[groupedPayee].paid - res[groupedPayee].share;
      }
    }
    summary = [];
